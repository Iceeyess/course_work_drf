    ...
    def __call__(self, value):
        ...
        if value.get('is_pleasant_sign'):
            if related_habit or award:
                raise serializers.ValidationError(f'Нельзя указать {field1} или {field2} '
                                                  f'при признаке приятной привычки.')
    ...
    def __call__(self, value):
        ...
        if related_habit:
            if not related_habit.is_pleasant_sign:
                raise serializers.ValidationError('Для связанной привычки должен быть признак приятной привычки.')
